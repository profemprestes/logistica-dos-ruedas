-- ============================================================================
--  PASO 12 — Arregla "Ya lo retiré" y hace obligatorio el retiro
--  Pegá TODO en Supabase → SQL Editor → Run. Se puede correr más de una vez.
--
--  1) BUG: `set_shipment_status` comparaba el estado (que es un enum) contra
--     texto, y Postgres no tiene ese operador:
--        operator does not exist: shipment_status = text
--     Faltaba el casteo. Es el error que sale al tocar "Ya lo retiré".
--
--  2) No se puede entregar un envío que nunca se retiró. Antes la app dejaba
--     saltear el paso y el seguimiento quedaba con un agujero.
-- ============================================================================
...
begin
  ...
  -- Acá estaba el bug: `v_shipment.status` es enum y `p_status` es text.
  -- Se compara contra la variable ya casteada.
  ...
  -- `scan_and_assign` ya deja su propio movimiento de retiro: no lo duplicamos.
...
-- ------------------------------------------- 2. no se entrega sin retirar antes
